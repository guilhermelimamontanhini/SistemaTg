package tg.Relatorios.TodosAlunosDoCurso;
...
@RestController
@RequestMapping("/todosAlunos")
@Service
public class RelatorioTodosAlunosDoCurso {
	
	@Autowired
	private AtiradorRepository atiradorRepository;
	
	@GetMapping
	public ResponseEntity<byte[]> gerarRelatorioTodosAlunosDoCurso() {
		
		ClassLoader chamarArquivo = getClass().getClassLoader();
		File nomeArquivo = new File(chamarArquivo.getResource("Relatorios/TodosAlunos.jrxml").getFile());
		
		List<Atirador> listaAtiradores = new ArrayList<>();
		
		listaAtiradores = this.atiradorRepository.findAllByOcupacaoOrderByNomeAsc("C");
