package tg.Relatorios.TodosMonitores;
...
@RestController
@RequestMapping("/todosMonitores")
@Service
public class RelatorioTodosMonitores {
	...
	public ResponseEntity<byte[]> gerarRelatorioTodosAlunosDoCurso() {
		...
		File nomeArquivo = new File(chamarArquivo.getResource("Relatorios/TodosMonitores.jrxml").getFile());
		
		List<Atirador> listaAtiradores = new ArrayList<>();
		
		listaAtiradores = this.atiradorRepository.findAllByOcupacaoOrderByNomeAsc("M");
